...
async function execInContainer(containerName, cmd, timeoutMs = 5000) {
  try {
    ...
    // Start exec and capture raw output
    const output = await new Promise((resolve, reject) => {
      const timer2 = setTimeout(() => reject(new Error('timeout')), timeoutMs);
      const req = http.request({
        socketPath: '/var/run/docker.sock',
        ...
        let data = '';
        res.on('data', (chunk) => { data += chunk; });
        res.on('end', () => { clearTimeout(timer2); resolve(data); });
      });
      req.on('error', (err) => { clearTimeout(timer2); reject(err); });
